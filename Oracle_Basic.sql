...
select count(*) from employees; --107사원

--사번, 이름 (last_name), 부서번호, 부서이름을 출력하세요. > 106사원
...
select e.employee_id, e.last_name, d.department_id, d.department_name, l.location_id, l.country_id
...
--다시 KOSA계정
select user from dual;
--Non-Equi join (비등가 조인) 1:1 매핑되는 칼럼이 없다.
--개념만 존재하고 
select * from emp;
select * from salgrade;
...
--정규화를 하면 SELECT성능이 낮아짐.(유지보수 낮아짐)
--여러칼럼에서 가져오는 것보다 한개의 컬럼에서 모두 가져오는게 더 빠름. (한개가 부담되더라도)
--테이블은 최대한 나눠준다. BUT, 현업에서 빠른 속도를 원하면 하나로 합치는 것이 낫다.

--OUTER JOIN(EQUI조인 선행되고 > 남아있는 데이터를 가져오는 방법
--주종관계 파악필요(주인이 되는 쪽에 남아있는 데이터를 가져오는 방법)
--1.left outer join
--2.right outer join
--3.full outer join (left, right > union)
select *
from m left outer join s --m이 주
on m.m1 = s.s1;

select *
from m right outer join s --s이 주
on m.m1 = s.s1;

select *
from m full outer join s --m이 주
on m.m1 = s.s1;
...
--사번, 이름 (last_name), 부서번호, 부서이름을 출력하세요. > null사원까지 107사원
...
-- 5. 직위(직종)가 'SALESMAN'인 사원들의 직종와 그 사원이름, 그리고
...
-- 6. 커미션(수당)이 책정된 사원들의 사원번호, 이름, 연봉, 연봉+커미션,
...
select d.empno as "사원번호", d.ename as "사원이름", e.empno as "관리자번호", e.ename as "관리자이름"
from emp d join emp e on e.deptno = d.deptno;

-------------------------------------------------------------------------------
-- subquery(SQL 꽃 ...만능해결사)
--함수 > 단일 테이블
--다중테이블(JOIN,UNION)
--해결안돼요 하는 데이터를 가진 테이블이 없어요
--가상테이블 >subquery(in line view)
...
인라인 뷰 : from 절 : view처럼 임시테이블(가상) ....
중첩 서브쿼리 : where 절 : 다중 컬럼 또는 다중 행을 반환
*/

/*
1.single row subquery : 실행 결과가 단일컬럼에 단일로우(한개의값)
...
문법) 
1. 괄호안에 있어야 한다.(쿼리문)
2. 단일 컬럼(select max(sal),min(sal) from emp 쓰지마세요)
3. 서브쿼리 단독실행 가능

실행순서 : 서브쿼리 실행되고 그 결과를 가지고 메인쿼리(주쿼리) 실행
*/

--사원테이블에서 jones의 급여보다 더 많은 급여를 받는 사원의 사번, 이름, 급여 출력
...
--부하직원이 있는 사번과 이름을 출력하세요.(내 empno 사번이 mgr에 있으면 나는 최소 한명의 사수)
select mgr from emp;
...
--부하직원이 없는 사원의 모든 정보 출력하세요.(내 empno 사번이 mgr에 있으면 나는 최소 한명의 사수)
select mgr from emp;
...
--and연산의 NULL은 모두 null로 만듬

select *
from emp
where empno not in (select nvl(mgr,0) from emp);
--nvl()로 null을 0으로 변경 후 포함유무 판단


--20번 부서의 사원중에서 가장 많은 월급을 받는 사원보다 더 많은 급여를 받는 사원의 사번, 이름, 급여, 부서번호 출력
...
-- 위 와 같은 JOIN 으로 해결 .... -> 현업에서는 join을 거의 사용
...
--자기 부서의 평균월급보다 더 많은 월급을 받는 사원의 사번 , 이름 , 부서번호 , 부서별 평균월급을 출력하세요
--만약에 부서와 부서별 평균월급 담고 있는 테이블 제공

-- 이재건
select e.empno,e.ename,e.deptno,e2.avf
from emp e join (
select deptno,avg(sal) as avf from emp
group by deptno) e2
...
--채번
select board_num.nextval from dual; --4번 돌렸을때 / 채번
select board_num.currval from dual; --4번까지 뽑음 / 현재 채번한 마지막 번호 확인
...
value(kboard_num.nextval,'1번');

insert into kboard(num,title)
value(kboard_num.nextval,'2번');

insert into kboard(num,title)
value(kboard_num.nextval,'3번');
...
insert into kboard(num,title)
value(kboard_num.nextval,'4번'); --3번까지 뽑혔기 때문에 그대로 4번이 뽑힘
...
start with 10
increment by 2; --10부터 2씩 증가 하는 시퀀스

select seq_num.nextval from dual; -- 10 12 14 16...
...
--create table 생성안됨
--rownum : 실제로 테이블에 컬럼으로 존재하지 않지만 내부적으로 행번호를 부여하는 칼럼
...
--Top-n 쿼리 (기준이 되는 데이터 생성 (정렬) ... 상위 n개 가져오기 
...
totaldata = 102
pagesize =10 (한 화면에 보여지는 데이터 row수)
...
--1단계 (데이터 정렬의 기준을 잡는다) : 사번
select * from employees
order by employee_id asc;


--2단계 (기준 데이터 순번 부여한다) 
select rownum as nu, e.*
from (
        select * from employees order by employee_id asc;
    ) e
where rownum <= 50; --이렇게도 가능


--3단계 (50과 41이 바뀌어지는 값이 되어야함 50/41 10/1 20/11)
select *
from (
        select rownum as nu, e.*
        from (
...
-- where rownum <= ?/where num >=?; ( 10과 1 부분의 값은 자바코드 논리 생성 parameter)


--프론트에서 페이지네이션을 캐싱하는 것은 동기화가 되지 않기때문에 안쓰는 것이 좋음
select *
from (
    SELECT ROW_NUMBER () OVER (ORDER BY employee_id ASC) AS num, e.*
    FROM employees e
    )n where n.num between 1 and 10;
...
--fetch next : 가져올 개수
--예 : OFFSET 1O ROWS FETCH NEXT 10 ROWS ONLY; -> 11번째부터 20번째까지
--------------------------------------------------------------------------------
...
*/
--View는 가상테이블 (물리적 테이블을 볼 수 있는 쿼리문을 가지고 있는 객체)
--사용 방법은 테이블처럼 ^^!
--view를 통해서 원 테이블 변경 가능


--1.보안 측면 (신입 사원이 입사하면 특정 컬럼 볼 수 있게 view 제공)
--2.편리성 (in line view : 필요할때마다 만들지만, view는 만들어두고 재사용 가능)
...
select e.empno , e.ename, e.deptno , e.sal , s.avgsal
from emp e join 
       (select deptno , trunc(avg(sal)능,0) as avgsal from emp group by deptno) s
on e.deptno = s.deptno
where e.sal > s.avgsal;
*/
...
-- SYSTEM > 다른 사용자 > KOSA > 오른쪽 버튼 사용자 편집 > 시스템 권한
-- > CREACT ANY VIEW 권한 부여(두개다 체크)

--이 설정을 해야 위에 코드 실행 가능
...
--select comm from view001; view가 볼 수 있는 데이터만 처리 가능
...
--혹자는, view는 SQL 문장 더어리 ...
--그러나 테이블처럼 사용이 가능하다.

--in line view > view 객체로 만들어서 join 하는 것이 낫다.
--view는 사용방법은 다 테이블처럼 사용하지만, 내부에는 데이터가 없다. 
--원본 table이 바뀌더라도 view는 바뀌지 않는다는 뜻이다. view는 구멍이 뚫려있는 아이일뿐.

--직종별 평균급여를 볼 수 있는 view 생성
create or replace view view_003
as
    select job as 직종 , trunc(avg(sal)) as 평균급여 --이름을 지정해줘야 가능
    from emp
    group by job;
...
--view를 통한 DML 작업
update v_emp set sal=0; --v_emp에서 볼수있는 원본 copyemp데이터를 바꾼것임.

select * from v_emp;

--deptno를 찾으려면 찾을 수 없음.
--왜냐하면 view를 통해서 볼 수 잇는 데이터 (select, insert, update, delete)가능
...
as
    select job as 직종, ename as 사원이름, sal as 월급
    from emp
...
--case문 활용
--row 데이터를 column 데이터로 전환
select deptno, case when deptno = 10 then 1 else 0 end dept_10 --이 컬럼으로 데이터 판단
            ,case when deptno = 20 then 1 else 0 end dept_20
            ,case when deptno = 30 then 1 else 0 end dept_30
from emp
order by deptno asc;

select deptno, sum(case when deptno = 10 then 1 else 0 end)as dept_10 --이 컬럼으로 데이터 판단
            ,sum(case when deptno = 20 then 1 else 0 end)as dept_20
            ,sum(case when deptno = 30 then 1 else 0 end)as dept_30
from emp
group by deptno --집계함수 group by 필요
order by deptno asc;

--deptno 컬럼은 의미가 없다.
--집중 (컬럼이름에 의미를 부여)

select sum(case when deptno = 10 then 1 else 0 end)as dept_10 --이 컬럼으로 데이터 판단
        ,sum(case when deptno = 20 then 1 else 0 end)as dept_20
        ,sum(case when deptno = 30 then 1 else 0 end)as dept_30
from emp;
...
----------------------------
--통계 집계 데이터 만들기
select case when deptno = 10 then cnt else 0 end dept_10,
    case when deptno = 20 then cnt else 0 end dept_20,
    case when deptno = 30 then cnt else 0 end dept_30
from (
    select deptno, count(*) as cnt
    from emp
    group by deptno
